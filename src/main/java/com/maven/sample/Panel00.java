...
public class Panel00 extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			/*
			 * g.drawLine((int)sx, (int)y, (int)p2d[0], (int)p2d[1]); g.fillRect((int)sx -
			 * 2, (int)y - 2, 4, 4); g.drawLine((int)p2d[0], (int)p2d[1], (int) (p2d[0] +
			 * Math.cos(da) * 100), (int)(p2d[1] + Math.sin(da) * 100)); //
			 */
			if (sx >= Math.min(x2, x1) && sx <= Math.max(x1, x2) && Math.min(y1, y2) <= y && Math.max(y1, y2) >= y) {
				...
				double tiny = 0.0001;
				double futuredist = Math.sqrt(Math.pow(sx - (p2d[0] + Math.cos(ba) * tiny), 2)
						+ Math.pow(y - (p2d[1] + Math.sin(ba) * tiny), 2));

				if (interdist <= bmag + r && futuredist < interdist) {
					// System.out.println("Carl Sagan" + i); //this is a pun because he wrote a book
					// called Contact
					if (i > preset.length) {
		...
		// g.drawLine((int) p2d[0], (int) p2d[1], (int) (p2d[0]+10*v2d[0]), (int)
		// (p2d[1]+10*v2d[1]));
